...
-- [[ INTEGRATED LOGIC FROM SECOND DOC ]]
do
task.spawn(function()
...
_G.SelectedToolsSeaEvent = {"Blox Fruit", "Melee", "Gun", "Sword"}
...
Net = require(Modules:WaitForChild("Net"));
...
Raidslist = {"Flame","Ice","Quake","Light","Dark","Spider","Magma","Buddha","Sand","Phoenix","Dough"}
...
function getFindItemBackpack(v)
	for i, v in pairs(LocalPlayer.Backpack:GetChildren()) do
		if string.find(v.Name, v) then
			return v
		end
	end
	for i, v in pairs(LocalPlayer.Backpack:GetChildren()) do
...
function TweenPlayer(Pos)
    if Character and Humanoid.Health > 0 and Character:FindFirstChild("HumanoidRootPart") then
        ...
        isTeleporting = true
        local Tween = game:GetService("TweenService"):Create(Character.PartTele, TweenInfo.new(Distance / _G.PlayerTweenSpeed, Enum.EasingStyle.Linear), {CFrame = Pos})
        Tween:Play()
...
function getClosestEnemy(distance)
	local target = nil
	local others = {}
...
function AttackEnemy(Target, Table)
	if Target then
		pcall(function()
			...
			RegisterHit:FireServer(Target, Table)
		end)
	end
end
...
function AttackNearest()
	pcall(function()
		local target, others = getClosestEnemy(AttackDistance)
		AttackEnemy(target, others)
	end)
end
function BladeHits()
	AttackNearest();
...
spawn(function()
	pcall(function()
		while wait() do
			if _G.AutoFarmLevel or _G.AutoFarmChestTween or _G.AutoElectricClaw or _G.AutoFarmFruitMastery or _G.AutoFarmGunMastery or _G.TeleportIsland or _G.TeleportToPlayer or _G.TweenToFruit or _G.TeleportNPC or _G.AutoFarmMon or _G.AutoFarmAllBoss or _G.AutoFarmBoss or _G.AutoFarmSwordMastery or _G.AutoFarmMaterial or _G.AutoAttackMonDungeon or _G.AutoFarmCandy or _G.AutoFarmSeabeast or _G.AutoFarmNearest or _G.AutoFarmHeart or _G.AutoCupidQuest then
				if not HumanoidRootPart:FindFirstChild("BodyClip") then
					local Noclip = Instance.new("BodyVelocity");
					Noclip.Name = "BodyClip";
					Noclip.Parent = HumanoidRootPart;
...
function MasteryAttackTarget(v, bring, stateFn)
	if not stateFn() then
		return
	end
	if not v or not v.Parent or not v:FindFirstChild("Humanoid") or v.Humanoid.Health <= 0 or _G.StopTween == true then
...
Tabs.CreditsTab:DiscordInfo({
    Name = "Relz Hub",
    Banner = "rbxassetid://129197321392617",
    Icon = "rbxassetid://102268449481061",
    Subtitle = "Relz Hub Script keeps things simple and smooth, with a clean interface and well-organized code that feels good to use.",
    Online = 283,
    Members = 6400,
    Callback = function()
...
AutoFarmSection = Tabs.MainTab:Section({
	Title = "Auto Farm",
	Icon = "sword",
	Opened = true
});
...
-- [[ AUTO FARM SYSTEM - REFACTORED ]] --
_G.Level = _G.AutoFarmLevel
_G.AutoFarmNear = _G.AutoFarmNearest

AutoLevelFarmToggle = AutoFarmSection:Toggle({
	Title = "Auto Farm Level",
	Desc = "Smart Quest & Attack Flow",
	Default = false,
...
AutoNearestFarmToggle = AutoFarmSection:Toggle({
	Title = "Auto Farm Nearest",
	Desc = "Locks and Kills Nearest Target",
	Default = false,
	Callback = function(state)
...
spawn(function()
	while wait(_G.GlobalDelay or 0.2) do
		if _G.AutoFarmLevel or _G.Level then
			pcall(function()
				CheckQuest();
				...
				if LocalPlayer.PlayerGui.Main.Quest.Visible == false then
					...
					if (HumanoidRootPart.Position - CFrameQuest.Position).Magnitude <= 5 then
						...
						for i, v in pairs(Enemies:GetChildren()) do
							if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v.Name == Mon then
								if string.find(QuestTitle, NameMon) then
									repeat
										wait()
										AttackTarget(v, true, function() return _G.AutoFarmLevel or _G.Level end)
									until not (_G.AutoFarmLevel or _G.Level) or not v.Parent or v.Humanoid.Health <= 0 or LocalPlayer.PlayerGui.Main.Quest.Visible == false
								else
									Remotes.CommF_:InvokeServer("AbandonQuest");
								end;
							end;
...
function getSpawnedBoss()
	for i, boss in pairs(tableBoss) do
		if Enemies:FindFirstChild(boss) then
			return Enemies:FindFirstChild(boss)
		end
	end
	return nil
end
function getReplicatedStorageBoss()
	for i, boss in pairs(tableBoss) do
		if ReplicatedStorage:FindFirstChild(boss) then
			return ReplicatedStorage:FindFirstChild(boss)
		end
	end
	return nil
end
...
AutoFruitMasteryToggle = MasteryFarmSection:Toggle({
	...
	Desc = "Auto Attack Mon With Fruit",
	Default = false,
	Callback = function(state)
...
AutoSwordMasteryToggle = MasteryFarmSection:Toggle({
	...
	Desc = "Auto Attack Mon With Sword",
	Default = false,
	Callback = function(state)
...
spawn(function()
	while wait() do
		wait(_G.GlobalDelay)
		if _G.AutoFarmSwordMastery and _G.MasteryMethod == "Level" then
			pcall(function()
				CheckQuest();
				local QuestTitle = LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text;
				if not string.find(QuestTitle, NameMon) then
...
spawn(function()
	while task.wait(0.2) do
		if _G.AutoFarmFruitMastery and _G.MasteryMethod == "Level" then
			pcall(function()
				CheckQuest();
				local QuestTitle = LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text;
				if not string.find(QuestTitle, NameMon) then
...
spawn(function()
	while wait() do
		wait(_G.GlobalDelay)
		if _G.AutoFarmGunMastery and _G.MasteryMethod == "Level" then
			pcall(function()
				CheckQuest();
				local QuestTitle = LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text;
				if not string.find(QuestTitle, NameMon) then
...
if World3 then
	...
	spawn(function()
		while wait(1) do
			TyrantOfTheSkiesEyesParagraph:SetDesc(checkEagleEye().."/4")
		end;
	end);
	...
	AutoSummonTyrantOfTheSkiesToggle = TyrantOfTheSkiesBossSection:Toggle({
		Title = "Auto Boss",
		Desc = "Auto Tyrant Of The Skies",
		Default = false,
		Callback = function(state)
			_G.AutoTyrantOfTheSkies = state;
			StopTween(_G.AutoTyrantOfTheSkies);
		end
	});
	spawn(function()
		while wait() do
			wait(_G.GlobalDelay)
			if _G.AutoTyrantOfTheSkies then
				pcall(function()
					if not Enemies:FindFirstChild("Tyrant of the Skies") then
							...
							for i, v in pairs(Enemies:GetChildren()) do
								if v.Name == "Serpent Hunter" or v.Name == "Skull Slayer" or v.Name == "Isle Champion" or v.Name == "Sun-kissed Warrior" then
									if v.Humanoid and v.Humanoid.Health > 0 then
										AttackTarget(v, true, function()
											return _G.AutoTyrantOfTheSkies
										end)
									end;
								end;
							end;
						...
						AttackTarget(Enemies:FindFirstChild("Tyrant of the Skies"), true, function()
...
AutoCollectBerryToggle = BerrySection:Toggle({
	...
	Desc = "Teleport To Collect Berry When Spawn",
	Default = false,
	Callback = function(state)
...
AutoFarmBossToggle = BossSection:Toggle({
	...
	Desc = "Auto Attack Selected Mon ( Need Spawn )",
	Default = false,
	Callback = function(state)
...
AutoFarmAllBossToggle = BossSection:Toggle({
	...
	Desc = "Auto Attack All Spawned Boss",
	Default = false,
	Callback = function(state)
...
spawn(function()
	while wait() do
		...
		if _G.AutoFarmAllBoss then
			pcall(function()
				local boss = getSpawnedBoss()
				if boss then
					AttackTarget(boss, true, function()
						return _G.AutoFarmAllBoss
					end)
				else
					TweenPlayer(getReplicatedStorageBoss().HumanoidRootPart.CFrame);
				end;
			end);
		end;
	end;
...
if World3 then
	...
	AutoEliteHunterToggle = EliteHunterSection:Toggle({
		...
		Desc = "Auto Attack Elite Mon ( Diablo | Deandre | Urban ) When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoEliteHunterHopToggle = EliteHunterSection:Toggle({
		...
		Desc = "Auto Hop Server When Elite Mon Doesn't Appear",
		Default = false,
		Callback = function(state)
	...
	AutoPirateRaidToggle = PirateRaidSection:Toggle({
		...
		Desc = "Auto Attack Raid Mon When it Comes",
		Default = false,
		Callback = function(state)
...
if World3 then
	...
	AutoKatakuriToggle = CakePrinceSection:Toggle({
		Title = "Auto Katakuri",
		Desc = "Auto Attack Cake Mon & Kill Cake Prince When Spawned",
		Default = false,
		Callback = function(state)
...
AutoFarmMaterialToggle = MaterialsSection:Toggle({
	...
	Desc = "Auto Attack Mon at the Place the Selected Material Appears",
	Default = false,
	Callback = function(state)
...
ChooseWeaponDropdown = SettingsSection:Dropdown({
	Title = "Weapon",
	Desc = "Select Weapon",
	Options = WeaponList,
...
BringMobNotes = SettingsSection:Paragraph({
	Title = "Notes",
	Desc = "If you want smoothly bring the monster please use budha transform"
})
BringMobToggle = SettingsSection:Toggle({
	Title = "Bring Mob",
	Desc = "Makes Surrounding Monsters Gather",
	Default = true,
	Callback = function(state)
		_G.BringMob = state;
	end
			...
			pcall(function()
				for i, v in pairs(Enemies:GetChildren()) do
					if (v.HumanoidRootPart.Position - HumanoidRootPart.Position).Magnitude <= 10000 then
						if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Name == MonFarm and (v.HumanoidRootPart.Position - HumanoidRootPart.Position).Magnitude <= _G.BringDistance then
...
BringDistanceSlider = SettingsSection:Slider({
	Title = "Bring Distance",
	Step = 1,
	Value = {
		Min = 100,
		Max = 1000,
		Default = _G.BringDistance or 500
	},
	Callback = function(value)
...
AttackAuraToggle = SettingsSection:Toggle({
	...
	Default = _G.AttackAura or false,
	Callback = function(state)
		_G.AttackAura = state;
	end
});
...
PlayerTweenSpeedSlider = SettingsSection:Slider({
	...
	Value = {
		Min = 50,
		Max = 350,
		Default = _G.PlayerTweenSpeed or 300
	},
	Callback = function(value)
...
HideNotificationToggle = GraphicSettingSection:Toggle({
	...
	Desc = "Make Notification Frame Invisible",
	Default = false,
	Callback = function(state)
...
HideDamageTextToggle = GraphicSettingSection:Toggle({
	...
	Desc = "Make Damage Frame Invisible",
	Default = _G.HideDamageText,
	Callback = function(state)
...
AutoSetSpawnPointToggle = OthersSettingsSection:Toggle({
	...
	Desc = "Auto Set Spawn Point When Teleport",
	Default = true,
	Callback = function(state)
...
spawn(function()
	while wait() do
		...
		if _G.AutoObservation then
			local dodgeDefault = LocalPlayer:GetAttribute("KenDodgesLeft")
			if dodgeValue > 0 then
				Remotes.CommE:FireServer("Ken", true);
			end;

			if World1 then
				if Enemies:FindFirstChild("Galley Captain") then
					repeat wait()
						HumanoidRootPart.CFrame = Enemies:FindFirstChild("Galley Captain").HumanoidRootPart.CFrame * CFrame.new(3,0,0)
					until _G.AutoObservation == false or dodgeValue <= 0
				else
					TweenPlayer(CFrame.new(5533.29785, 88.1079102, 4852.3916))
				end
			elseif World2 then
				if Enemies:FindFirstChild("Lava Pirate") then
					repeat wait()
						HumanoidRootPart.CFrame = Enemies:FindFirstChild("Lava Pirate").HumanoidRootPart.CFrame * CFrame.new(3,0,0)
					until _G.AutoObservation == false or dodgeValue <= 0
				else
					TweenPlayer(CFrame.new(-5478.39209, 15.9775667, -5246.9126))
				end
			elseif World3 then
				if Enemies:FindFirstChild("Venomous Assailant") then
					repeat wait()
						HumanoidRootPart.CFrame = Enemies:FindFirstChild("Venomous Assailant").HumanoidRootPart.CFrame * CFrame.new(3,0,0)
					until _G.AutoObservation == false or dodgeValue <= 0
				else
					TweenPlayer(CFrame.new(4530.3540039063, 656.75695800781, -131.60952758789))
				end
			end
		end;
	end;
...
AutoRejoinToggle = OthersSettingsSection:Toggle({
	Title = "Auto Rejoin",
	Desc = "Auto Rejoin When Disconnected",
	Default = true,
	Callback = function(state)
		_G.AutoRejoin = state;
	end
...
if World3 then
	SettingSeaSection = Tabs.SettingsTab:Section({
		Title = "Sea Event",
		Icon = "anchor"
	});
	...
	SelectedToolsSeaEventDropdown = SettingSeaSection:Dropdown({
		Title = "Tools",
		Desc = "Select Tools For Sea Event",
		Options = toolsSeaEvent,
		Default = _G.SelectedToolsSeaEvent,
		Multi = true,
		Callback = function(value)
			_G.SelectedToolsSeaEvent = value;
		end
	});

	DevilFruiSKillSeaEventDropdown = SettingSeaSection:Dropdown({
		Title = "Devil Fruit",
		Desc = "Select Devil Fruit Skill",
		Options = FruitSkillList,
		Default = _G.DevilFruitSkillSeaEvent,
		Multi = true,
		Callback = function(value)
	...
	MeleeSkillSeaEventDropdown = SettingSeaSection:Dropdown({
		Title = "Melee",
		Desc = "Select Melee Skill",
		Options = MeleeSkillList,
		Default = _G.MeleeSkillSeaEvent,
		Multi = true,
		Callback = function(value)
	...
	spawn(function()
		while wait() do
			...
			pcall(function()
				if _G.SeaSkill then
					if table.find(_G.SelectedToolsSeaEvent, "Blox Fruit") and DoneSkillFruit == false then
					...
					if table.find(_G.SelectedToolsSeaEvent, "Melee") and DoneSkillMelee == false then
					...
					if table.find(_G.SelectedToolsSeaEvent, "Sword") and DoneSkillSword == false then
					...
					if table.find(_G.SelectedToolsSeaEvent, "Gun") and DoneSkillGun == false then
...
if World1 or World2 then
	WorldSection = Tabs.OthersTab:Section({
		Title = "World",
		Icon = "globe"
	});
end
...
if World3 then
	...
	CursedDualKatanaParagraph = GunSwordSection:Paragraph({
	...
	spawn(function()  
		while wait(_G.GlobalDelay) do 
			if QuestYama_1 == true then 
				CursedDualKatanaParagraph:SetDesc("Status : Yama Quest 1") 
			elseif QuestYama_2 == true then
				CursedDualKatanaParagraph:SetDesc("Status : Yama Quest 2") 
			elseif QuestYama_3 == true then
				CursedDualKatanaParagraph:SetDesc("Status : Yama Quest 3") 
			elseif QuestTushita_1 == true then
				CursedDualKatanaParagraph:SetDesc("Status : Tushita Quest 1") 
			elseif QuestTushita_2 == true then
				CursedDualKatanaParagraph:SetDesc("Status : Tushita Quest 2") 
			elseif QuestTushita_3 == true then
				CursedDualKatanaParagraph:SetDesc("Status : Tushita Quest 3") 
			elseif getOwned("Cursed Dual Katana") then
				CursedDualKatanaParagraph:SetDesc("CDK Done!!")
			end 
		end
	end) 
	AutoGetCursedDualKatanaToggle = GunSwordSection:Toggle({
		Title = "Auto Get CDK",
		Desc = "Complete automatically",
		Default = false,
		Callback = function(state)
	...
	AutoQuestCursedDualKatanaYamaToggle = GunSwordSection:Toggle({
	...
	AutoQuestCursedDualKatanaTushitaToggle = GunSwordSection:Toggle({
...
if World1 then
	AutoGreybeardToggle = GunSwordSection:Toggle({
...
if World1 then
	Tabs.RaidTab:Paragraph({
		Title = "Sea 2 or Sea 3 Only",
		Desc = "Please Teleport To Sea 2 or Sea 3 First"
	})
end
if World3 then
	...
	function getMyIslandRaidMap(island)
		for i, v in pairs(Workspace.Map.RaidMap:GetChildren()) do
			if v.Name == island and (v.WorldPivot.Position - HumanoidRootPart.Position).Magnitude <= 3000 then
				return v;
			end;
		end;
		return nil;
	...
	function NextRaidIsland()
		...
		if LocalPlayer.PlayerGui.Main.TopHUDList.RaidTimer.Visible == true then
			...
			elseif getMyIslandRaidMap("RaidIsland5") then
				TweenPlayer(CFrame.new((getMyIslandRaid("RaidIsland5")).WorldPivot.Position) * RaidPos);
			elseif getMyIslandRaid("Island 4") then
				TweenPlayer((getMyIslandRaid("Island 4")).CFrame * RaidPos);
			elseif getMyIslandRaidMap("RaidIsland4") then
				TweenPlayer(CFrame.new((getMyIslandRaid("RaidIsland4")).WorldPivot.Position) * RaidPos);
			elseif getMyIslandRaid("Island 3") then
				TweenPlayer((getMyIslandRaid("Island 3")).CFrame * RaidPos);
			elseif getMyIslandRaidMap("RaidIsland3") then
				TweenPlayer(CFrame.new((getMyIslandRaid("RaidIsland3")).WorldPivot.Position) * RaidPos);
			elseif getMyIslandRaid("Island 2") then
				TweenPlayer((getMyIslandRaid("Island 2")).CFrame * RaidPos);
			elseif getMyIslandRaidMap("RaidIsland2") then
				TweenPlayer(CFrame.new((getMyIslandRaid("RaidIsland2")).WorldPivot.Position) * RaidPos);
			elseif getMyIslandRaid("Island 1") then
				TweenPlayer((getMyIslandRaid("Island 1")).CFrame * RaidPos);
			elseif getMyIslandRaidMap("RaidIsland1") then
				TweenPlayer(CFrame.new((getMyIslandRaid("RaidIsland1")).WorldPivot.Position) * RaidPos);
			end;
		end;
	end;
...
if World2 then
	...
	AutoLawRaidToggle = LawRaidSection:Toggle({
		Title = "Auto Law Raid",
		Desc = "Auto Attack Law Boss When Spawned",
		Default = false,
		Callback = function(state)
			_G.LawRaid = state;
...
if not World1 and not World2 then
	...
	AutoReturnToHubToggle = DungeonSection:Toggle({
...
if World1 or World2 then
	Tabs.RaceTab:Paragraph({
		Title = "Sea 3 Only",
		Desc = "Please Teleport To Sea 3 First"
	})
end
if World3 then
	TeleportRaceSection = Tabs.RaceTab:Section({
		Title = "Teleport",
		Icon = "map-pinned",
		Opened = true
	});
	...
	SelectedPlaceDropdown = TeleportRaceSection:Dropdown({
	...
	TeleportToPlaceToggle = TeleportRaceSection:Toggle({
		...
		pcall(function()
			while wait() do
				...
				if _G.AutoTrain then
					if Character.RaceTransformed.Default == true then
	...
	spawn(function()
		pcall(function()
			while wait(0.2) do
				if _G.AutoTrain then
					if Character.RaceTransformed.Default == false then
						StartFarmTrain = true;
					end;
				end;
			end;
...
WorldTeleportSection = Tabs.TeleportTab:Section({
	Title = "World",
	Icon = "globe",
	Opened = true
});
TeleportToFirstSeaButton = WorldTeleportSection:Button({
	Title = "Teleport To First Sea",
	Desc = "Travel to Sea 1",
	Callback = function()
		Remotes.CommF_:InvokeServer("TravelMain");
	end
});
TeleportToSecondSeaButton = WorldTeleportSection:Button({
	Title = "Teleport To Second Sea",
	Desc = "Travel to Sea 2",
	Callback = function()
		Remotes.CommF_:InvokeServer("TravelDressrosa");
	end
});
TeleportToThirdSeaButton = WorldTeleportSection:Button({
	Title = "Teleport To Third Sea",
	Desc = "Travel to Sea 3",
	Callback = function()
		Remotes.CommF_:InvokeServer("TravelZou");
	end
});
...
AutoBuyLegendarySwordToggle = ShopSection:Toggle({
	...
	Desc = "Auto purchase from Legendary Sword Dealer",
	Default = false,
	Callback = function(state)
...
BuyBlackLegToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Black Leg",
	Desc = "Auto travel to buy Black Leg ($150,000)",
	Default = false,
	Callback = function(state)
		_G.BuyBlackLeg = state
	end
});
...
spawn(function()
	while wait() do
		if _G.BuyBlackLeg then
			pcall(function()
				local npcPos = CFrame.new(-5043.64013671875, 371.3428955078125, -3183.39990234375)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuyBlackLeg");
				end
			end)
		end
...
BuyElectroToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Electro",
	Desc = "Auto travel to buy Electro ($550,000)",
	Default = false,
	Callback = function(state)
		_G.BuyElectro = state
	end
...
spawn(function()
	while wait() do
		if _G.BuyElectro then
			pcall(function()
				local npcPos = CFrame.new(-4993.2001953125, 314.5458679199219, -3198.06005859375)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuyElectro");
				end
...
BuyFishmanKarateToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Fishman Karate",
	Desc = "Auto travel to buy Fishman Karate ($750,000)",
	Default = false,
	Callback = function(state)
		_G.BuyFishmanKarate = state
	end
});

spawn(function()
	while wait() do
		if _G.BuyFishmanKarate then
			pcall(function()
				local npcPos = CFrame.new(-5017.39013671875, 371.3429260253906, -3187.530029296875)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuyFishmanKarate");
				end
...
BuyDragonClawToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Dragon Claw",
	Desc = "Auto travel to buy Dragon Claw (ƒ1,500)",
	Default = false,
	Callback = function(state)
		_G.BuyDragonClaw = state
	end
});

spawn(function()
	while wait() do
		if _G.BuyDragonClaw then
			pcall(function()
				local npcPos = CFrame.new(-5043.64013671875, 371.3428955078125, -3183.39990234375)
				-- TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
...
BuySuperhumanToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Superhuman",
	Desc = "Auto travel to buy Superhuman ($3,000,000)",
	Default = false,
	Callback = function(state)
		_G.BuySuperhuman = state
	end
});
spawn(function()
	while wait() do
		if _G.BuySuperhuman then
			pcall(function()
				local npcPos = CFrame.new(-4997.52978515625, 371.3428955078125, -3197.4599609375)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuySuperhuman");
				end
...
BuyDeathStepToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Death Step",
	Desc = "Auto travel to buy Death Step (ƒ5,000 $5,000,000)",
	Default = false,
	Callback = function(state)
		_G.BuyDeathStep = state
	end
});
spawn(function()
	while wait() do
		if _G.BuyDeathStep then
			pcall(function()
				local npcPos = CFrame.new(-4997.64013671875, 314.5458679199219, -3220.3701171875)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuyDeathStep");
				end
...
BuySharkmanKarateToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Sharkman Karate",
	Desc = "Auto travel to buy Sharkman Karate (ƒ5,000 $2,500,000)",
	Default = false,
	Callback = function(state)
		_G.BuySharkmanKarate = state
	end
});
spawn(function()
	while wait() do
		if _G.BuySharkmanKarate then
			pcall(function()
				local npcPos = CFrame.new(-4970.47998046875, 314.5458679199219, -3225.0400390625)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
...
BuyElectricClawToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Electric Claw",
	Desc = "Auto travel to buy Electric Claw (ƒ5,000 $3,000,000)",
	Default = false,
	Callback = function(state)
...
spawn(function()
	while wait() do
		if _G.BuyElectricClaw then
			pcall(function()
				local npcPos = CFrame.new(-10369.830078125, 331.6841125488281, -10126.490234375)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuyElectricClaw");
				end
...
BuyDragonTalonToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Dragon Talon",
	Desc = "Auto travel to buy Dragon Talon (ƒ5,000 $3,000,000)",
	Default = false,
	Callback = function(state)
		_G.BuyDragonTalon = state
	end
});
spawn(function()
	while wait() do
		if _G.BuyDragonTalon then
			pcall(function()
				local npcPos = CFrame.new(-4993.2001953125, 314.5458679199219, -3198.06005859375)
				-- TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
...
BuyGodHumanToggle = FightingStyleShopSection:Toggle({
	Title = "Buy God Human",
	Desc = "Auto travel to buy God Human (ƒ5,000 $5,000,000)",
	Default = false,
	Callback = function(state)
		_G.BuyGodhuman = state
	end
});
spawn(function()
	while wait() do
		if _G.BuyGodhuman then
			pcall(function()
				local npcPos = CFrame.new(-13775.5595703125, 334.65240478515625, -9877.669921875)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
					Remotes.CommF_:InvokeServer("BuyGodhuman");
				end
...
BuySanguineArtToggle = FightingStyleShopSection:Toggle({
	Title = "Buy Sanguine Art",
	Desc = "Auto travel to buy Sanguine Art (ƒ5,000 $5,000,000)",
	Default = false,
	Callback = function(state)
		_G.BuySanguineArt = state
	end
});
spawn(function()
	while wait() do
		if _G.BuySanguineArt then
			pcall(function()
				local npcPos = CFrame.new(-16514.859375, 23.1700382232666, -190.8429412841797)
				TweenPlayer(npcPos)
				if (npcPos.Position - HumanoidRootPart.Position).Magnitude <= 5 then
...
if World3 then
	BeltSection = Tabs.DragonDojoTab:Section({
		Title = "Belt",
		Icon = "zap",
		Opened = true
	});
	AutoDojoTrainerToggle = BeltSection:Toggle({
		Title = "Auto Dojo Trainer",
		Desc = "Upgrade Belt",
		Default = false,
		Callback = function(state)
			_G.AutoDojoTrainer = state;
			StopTween(_G.AutoDojoTrainer)
		end
	});
				...
				if _G.AutoDojoTrainer then
					pcall(
						function()
							...
							if not progress and not NameBelt then
							...
							elseif
									(CFrame.new(5865.0234375, 1208.3154296875, 871.15185546875).Position -
										HumanoidRootPart.Position).Magnitude <= 50
							then
								if NameBelt == "White" then
									local v = GetConnectionEnemies("Skull Slayer")
									if v then
										AttackTarget(v, false, function()
											return _G.AutoCursedDualKatana or not progress
										end)
									else
										TweenPlayer(CFrame.new(-16759.58984375, 71.28376770019531, 1595.3399658203125))
									end
								elseif NameBelt == "Yellow" then
									repeat
										...
										_G.AutoFarmShark = true
										_G.AutoFarmPiranha = true
										_G.AutoFarmFishCrewMember = true
										_G.AutoFarmPirateBrigade = true
										_G.AutoFarmPirateGrandBrigade = true
										_G.AutoFarmGhostShip = true
										_G.SailBoat = true
									until not _G.AutoDojoTrainer or not progress
									_G.AutoFarmSeabeast = false
									_G.AutoTerrorshark = false
									_G.AutoFarmShark = false
									_G.AutoFarmPiranha = false
									_G.AutoFarmFishCrewMember = false
									_G.AutoFarmPirateBrigade = false
									_G.AutoFarmPirateGrandBrigade = false
								...
								elseif NameBelt == "Black" then
									repeat
										...
										then
											...
											then
												_G.AutoKillLavaGolem = false
												_G.AutoDefendVolcano = false
												_G.AutoPrehistoricIsland = true
											else
												_G.AutoKillLavaGolem = true
	...
	VolcanicMagnetSection = Tabs.DragonDojoTab:Section({
		Title = "Volcanic Magnet",
		Icon = "magnet"
	});
	AutoFarmBlazeEmberToggle = VolcanicMagnetSection:Toggle({
	...
	CraftVolcanicMagnetButton = VolcanicMagnetSection:Button({
	...
	DracoTrialSection = Tabs.DragonDojoTab:Section({
		Title = "Draco Trial",
		Icon = "vegan"
	});
	...
	UpgradeDracoTrialToggle = DracoTrialSection:Toggle({
	...
	AutoDracoV1Toggle = DracoTrialSection:Toggle({
		Title = "Auto Draco V1",
	...
	AutoDracoV2Toggle = DracoTrialSection:Toggle({
		Title = "Auto Draco V2",
	...
	AutoDracoV3Toggle = DracoTrialSection:Toggle({
		Title = "Auto Draco V3",
		...
		while wait(_G.GlobalDelay) do
			pcall(function()
				if _G.AutoDracoV3 then     
					local dragonWizardQuest = true
					repeat wait()
						_G.SailBoat = true
						_G.AutoTerrorshark = true
						_G.SelectedZone = "Inf"
					until not dragonWizardQuest or not _G.AutoDracoV3
					_G.SailBoat = false
					_G.AutoTerrorshark = false
					_G.SelectedZone = "Zone 5"
				end
	...
	TeleportToDracoTrialsToggle = DracoTrialSection:Toggle({
	...
	SwapDracoRaceToggle = DracoTrialSection:Toggle({
	...
	UpgradeDragonTalonToggle = DracoTrialSection:Toggle({
...
if World3 then
	...
	AutoFarmSharkToggle = SeaEventEnemiesSection:Toggle({
		...
		Desc = "Auto Attack Shark When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmPiranhaToggle = SeaEventEnemiesSection:Toggle({
		...
		Desc = "Auto Attack Piranha When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmFishCrewMemberToggle = SeaEventEnemiesSection:Toggle({
		...
		Desc = "Auto Attack Fish Crew When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmGhostShipToggle = SeaEventBoatSection:Toggle({
		...
		Desc = "Auto Attack Ghost Ship When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmPirateBrigadeToggle = SeaEventBoatSection:Toggle({
		...
		Desc = "Auto Attack Pirate Brigade When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmPirateGrandBrigadeToggle = SeaEventBoatSection:Toggle({
		...
		Desc = "Auto Attack Grand Brigade When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmTerrorsharkToggle = SeaEventBossSection:Toggle({
		...
		Desc = "Auto Attack Terrorshark When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoFarmSeabeastsToggle = SeaEventBossSection:Toggle({
		...
		Desc = "Auto Attack Seabeast When Spawned",
		Default = false,
		Callback = function(state)
...
if World3 then
	...
	spawn(function()
		pcall(function()
			while wait(0.2) do
				if Workspace.Map:FindFirstChild("MysticIsland") then
					MirageStatusSeaStackParagraph:SetDesc("🏝️ Mirage Island Spawning");
				else
					MirageStatusSeaStackParagraph:SetDesc("🏝️ Mirage Island Not Spawn");
				end;
				if Workspace.Map:FindFirstChild("KitsuneIsland") then
					KitsuneStatusSeaStackParagraph:SetDesc("⛩️ Kitsune Island Spawning");
				else
					KitsuneStatusSeaStackParagraph:SetDesc("⛩️ Kitsune Island Not Spawn");
				end;
				if Workspace.Map:FindFirstChild("FrozenDimension") then
					FrozenStatusSeaStackParagraph:SetDesc("❄️ Frozen Dimension Spawning");
				else
					FrozenStatusSeaStackParagraph:SetDesc("❄️ Frozen Dimension Not Spawn");
				end;
	...
	AutoPrehistoricIslandToggle = PrehistoricSection:Toggle({
		Title = "Auto Prehistoric Island",
		Desc = "Teleport & Find Prehistoric Island",
		Default = false,
		Callback = function(state)
			_G.AutoPrehistoricIsland = state;
			StopTween(_G.AutoPrehistoricIsland);
		end
	});
	spawn(function()
		while wait() do
			wait(_G.GlobalDelay)
			pcall(function()
				if _G.AutoPrehistoricIsland and World3 then
					if not Workspace.Map:FindFirstChild("PrehistoricIsland") then
						...
						elseif Workspace.Boats:FindFirstChild(_G.SelectedBoat) then
							...
							until not _G.AutoPrehistoricIsland or Workspace.Map:FindFirstChild("PrehistoricIsland")
							if TweenBoatPrehistoric then
								TweenBoatPrehistoric:Stop();
							end;
						end;
					else
						TweenPlayer(CFrame.new(Workspace.Map:FindFirstChild("PrehistoricIsland").WorldPivot.Position))
					end
				end;
			end);
		end;
	...
	AutoKillLavaGolemToggle = PrehistoricSection:Toggle({
		...
		Desc = "Auto Attack Lava Golem When Spawned",
		Default = false,
		Callback = function(state)
	...
	AutoCollectBoneToggle = PrehistoricSection:Toggle({
	...
	AutoCollectDragonEggToggle = PrehistoricSection:Toggle({
	...
	AutoDefendVolcanoToggle = PrehistoricSection:Toggle({
				...
				if Workspace.Map:FindFirstChild("PrehistoricIsland") then
					...
					if rock then
						...
						if ((rockColor ~= Color3.fromRGB(185, 53, 56)) and (rockColor ~= Color3.fromRGB(185, 53, 57))) then
							...
							local rockDistance = ((HumanoidRootPart.Position - rock.Position)).Magnitude;
	...
	FrozenStatusSeaStackParagraph = FrozenDimensionSection:Paragraph({
		Title = "Frozen Status",
		Desc = "-"
	});
	AutoFrozenDimensionToggle = FrozenDimensionSection:Toggle({
		Title = "Auto Frozen Dimension",
		Desc = "Teleport & Find Frozen Deimension",
		Default = false,
		Callback = function(state)
			_G.AutoFrozenDimension = state;
		...
		while wait() do
			...
			pcall(function()
				if _G.AutoFrozenDimension and World3 then
					if not Workspace.Map:FindFirstChild("FrozenDimension") then
						if not Workspace.Boats:FindFirstChild(_G.SelectedBoat) then
							local BuyBoatCFrame = CFrame.new(-16927.451171875, 9.0863618850708, 433.8642883300781);
							if (BuyBoatCFrame.Position - HumanoidRootPart.Position).Magnitude > 2000 then
								BTP(BuyBoatCFrame);
						...
						elseif Workspace.Boats:FindFirstChild(_G.SelectedBoat) then
							...
							until not _G.AutoFrozenDimension or Workspace.Map:FindFirstChild("FrozenDimension")
							if TweenBoatFrozen then
								TweenBoatFrozen:Stop();
							end;
						end;
					else
						TweenPlayer(CFrame.new(Workspace.Map:FindFirstChild("FrozenDimension").WorldPivot.Position))
					end
				end;
			end);
		end;
	...
	AutoAttackLeviathanToggle = FrozenDimensionSection:Toggle({
	...
	KitsuneStatusSeaStackParagraph = KitsuneIslandSection:Paragraph({
		Title = "Kitsune Status",
		Desc = "-"
	});
	AutoKitsuneIslandToggle = KitsuneIslandSection:Toggle({
		Title = "Auto Kitsune Island",
		Desc = "Teleport & Find Kitsune Island",
		Default = false,
		Callback = function(state)
			_G.AutoKitsuneIsland = state;
			StopTween(_G.AutoKitsuneIsland);
		end
	});
	spawn(function()
		while wait() do
			wait(_G.GlobalDelay)
			pcall(function()
				if _G.AutoKitsuneIsland and World3 then
					if not Workspace.Map:FindFirstChild("KitsuneIsland") then
						...
						elseif Workspace.Boats:FindFirstChild(_G.SelectedBoat) then
							...
							until not _G.AutoKitsuneIsland or Workspace.Map:FindFirstChild("KitsuneIsland")
							if TweenBoatKitsune then
								TweenBoatKitsune:Stop();
							end;
						end;
...
if World2 or World3 then
	...
	MirageStatusSeaStackParagraph = MirageIslandSection:Paragraph({
		Title = "Mirage Status",
		Desc = "-"
	});
...
WalkSpeedSlider = LocalPlayerSection:Slider({
	Title = "Walk Speed",
	Step = 1,
	Value = {
		Min = 16,
		Max = 500,
		Default = _G.WalkSpeed or 100
	},
	Callback = function(value)
		_G.WalkSpeed = value;
	end
})
ModifyWalkSpeedToggle = LocalPlayerSection:Toggle({
	Title = "Enable Walk Speed",
	Default = false,
	Callback = function(state)
		_G.EnableWalkSpeed = state
	end
})
JumpPowerSlider = LocalPlayerSection:Slider({
	Title = "Jump Power",
	Step = 1,
	Value = {
		Min = 50,
		Max = 500,
		Default = _G.JumpPower or 200
	},
	Callback = function(value)
		_G.JumpPower = value;
	end
})
ModifyJumpPowerToggle = LocalPlayerSection:Toggle({
	Title = "Enable Jump Power",
	Default = false,
	Callback = function(state)
		_G.EnableJumpPower = state
	end
})
...
AutoRandomFruitToggle = FruitSection:Toggle({
	...
	Desc = "Auto Gacha Fruit at Blox Fruit Gacha",
	Default = false,
	Callback = function(state)
...
StoreRarityFruitDropdown = FruitSection:Dropdown({
	...
	Default = SelectStoreRarityFruits[1],
	Callback = function(option)
...
AutoStoreFruitToggle = FruitSection:Toggle({
	...
	Desc = "Auto Store Fruit to Inventory",
	Default = false,
	Callback = function(state)
...
TeleportToFruitToggle = FruitSection:Toggle({
	...
	Desc = "Instant TP to Fruit Spawn",
	Default = false,
	Callback = function(state)
...
TweenToFruitToggle = FruitSection:Toggle({
	...
	Desc = "Safe Tween to Fruit Spawn",
	Default = false,
	Callback = function(state)
...
GrabFruitButton = FruitSection:Button({
	Title = "Grab Fruit",
	Desc = "Pull nearby fruits to character",
	Callback = function()
		for i, v in pairs(Workspace:GetChildren()) do
			if v:IsA("Tool") then
...
RainFruitButton = VisualSection:Button({
	Title = "Rain Fruit",
	Desc = "Visual only: Make it rain fruits",
	Callback = function()
		RainFruit();
	end
});
...
RemoveLavaButton = GraphicMiscSection:Button({
	Title = "Remove Lava",
	Desc = "Delete lava parts ( careful )",
	Callback = function()
		for i, v in pairs(Workspace:GetDescendants()) do
			if v.Name == "Lava" then
				v:Destroy();
...
CopyJobIdButton = ServerTabSection:Button({
	Title = "Copy Job ID",
	Desc = "Copy server identifier to clipboard",
	Callback = function()
		setclipboard(game.JobId);
	end
});
EnterJobIdInput = ServerTabSection:Textbox({
	Title = "Enter Job ID",
	Desc = "Paste a server ID to join",
	Callback = function(value)
		_G.JobId = value;
	end
});
JoinJobIdButton = ServerTabSection:Button({
	Title = "Join Job ID",
	Desc = "Teleport to the specified server ID",
	Callback = function()
		RelzhubModule:JoinJobId(_G.JobId)
	end
});
...
-- [[ ORGANIZED EXTRA FEATURES BY SEA ]]
if World2 then
	local ExtraSea2 = Tabs.MainTab:Section({ Title = 'Sea 2 Extras', Icon = 'anchor', Opened = false })
		Tabs.MainTab:Toggle({ Title = "Auto Travel Dressrosa (Extra)", Callback = function(s) _G.TravelDres = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Factory Raid (Extra)", Callback = function(s) _G.AutoFactory = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Done Bartilo Quest (Extra)", Callback = function(s) _G.Bartilo_Quest = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Law Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Rengoku Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Dragon Trident (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Darkbeard (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
end
if World3 then
	local ExtraSea3 = Tabs.MainTab:Section({ Title = 'Sea 3 Extras', Icon = 'anchor', Opened = false })
		Tabs.MainTab:Toggle({ Title = "Auto Zou Quest (Extra)", Callback = function(s) _G.AutoZou = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Pirate Raid (Extra)", Callback = function(s) _G.AutoRaidCastle = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Done Citizen Quest (Extra)", Callback = function(s) _G.CitizenQuest = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Cake Prince (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Bones (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Farm Mirror (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Bigmom (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
end
local ExtraGeneral = Tabs.MainTab:Section({ Title = 'General Extras', Icon = 'plus', Opened = false })
		Tabs.MainTab:Toggle({ Title = "Auto Farm Level (Extra)", Callback = function(s) _G.Level = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Farm Nearest (Extra)", Callback = function(s) _G.AutoFarmNear = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Materials (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Farm Ectoplasm (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Training Dummy (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Berry (Extra)", Callback = function(s) _G.AutoBerry = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Chest (Extra)", Callback = function(s) _G.AutoFarmChest = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Mastery Fruits (Extra)", Callback = function(s) _G.FarmMastery_Dev = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Mastery Gun (Extra)", Callback = function(s) _G.FarmMastery_G = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Mastery All Sword (Extra)", Callback = function(s) _G.FarmMastery_S = s end })
		Tabs.MainTab:Toggle({ Title = "Accept Quests (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Soul Reaper [Fully] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Random Bones (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Try Luck Gravestone (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Pray Gravestone (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Unlock Dough dungeon (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Unlock Phoenix dungeon (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Teleport Barista Cousin (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Rainbow Colors (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Accept Rainbow Quest Faster (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Farm Observation (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Observation V2 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Upgrade Mink V3 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Upgrade Human V3 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Upgrade Skypiea V3 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Upgrade FishMan V3 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Valkyrie (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Unlocked Puzzle (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Initialize Attack [M1/Melee/Sword] (Extra)", Callback = function(s) _G.Seriality = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Turn on Race V3 (Extra)", Callback = function(s) _G.RaceClickAutov3 = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Turn on Race V4 (Extra)", Callback = function(s) _G.RaceClickAutov4 = s end })
		Tabs.MainTab:Toggle({ Title = "Turn on Bypass Teleport (Extra)", Callback = function(s) _G.Bypass = s end })
		Tabs.MainTab:Toggle({ Title = "Panic Mode (Extra)", Callback = function(s) _G.Safemode = s end })
		Tabs.MainTab:Toggle({ Title = "Anti AFK (Extra)", Callback = function(s) _G.AntiAFK = s end })
		Tabs.MainTab:Toggle({ Title = "Remove Hit VFX (Extra)", Callback = function(s) _G.DistroyHit = s end })
		Tabs.MainTab:Toggle({ Title = "Remove Death & Respawned VFX (Extra)", Callback = function(s) _G.DistroyHit = s end })
		Tabs.MainTab:Toggle({ Title = "Disable Notify (Extra)", Callback = function(s) _G.DistroyHit = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Melee (Extra)", Callback = function(s) _G.Auto_Melee = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Swords (Extra)", Callback = function(s) _G.Auto_Melee = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Gun (Extra)", Callback = function(s) _G.Auto_Melee = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Blox Fruit (Extra)", Callback = function(s) _G.Auto_Melee = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Defense (Extra)", Callback = function(s) _G.Auto_Melee = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Superhuman (Extra)", Callback = function(s) _G.Auto_SuperHuman = s end })
		Tabs.MainTab:Toggle({ Title = "Auto DeathStep (Extra)", Callback = function(s) _G.AutoDeathStep = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Sharkman Karate (Extra)", Callback = function(s) _G.Auto_SharkMan_Karate = s end })
		Tabs.MainTab:Toggle({ Title = "Auto ElectricClaw (Extra)", Callback = function(s) _G.Auto_Electric_Claw = s end })
		Tabs.MainTab:Toggle({ Title = "Auto DragonTalon (Extra)", Callback = function(s) _G.AutoDragonTalon = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Godhuman (Extra)", Callback = function(s) _G.Auto_God_Human = s end })
		Tabs.MainTab:Toggle({ Title = "Auto SanguineArt (Extra)", Callback = function(s) _G.snaguine = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Elite Quest (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Stop when got God's Chalice (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Tushita Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Yama Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Get CDK [ Last Quest ] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Yama CDK (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Tushita CDK (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Tween to Legendary Sword Dealer (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Pole V1 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Pole V2 [Patched] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Saw Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Saber Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Cybrog (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Usoap's Hat (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Bisento V2 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Warden Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Marine Coat (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Swan Coat (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Key Rengoku (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Long Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Black Spikey (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Dark Blade V3 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Midnight Blade (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Unlocked DonSwan (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Swan Glasses (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Canvendish Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Twin Hooks (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Serpent Bow (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Lei Accessory (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Valentine Gacha (Extra)", Callback = function(s) _G.AutoValentineGacha = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Find Mirage Island (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Tween To Highest Point (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Gear (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Change Transparency can see (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Tween Advanced Fruit Dealer (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Mirage Chest (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Skull Guitar (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Farm Material Skull Guitar (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Look At Moon (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Pull Lever (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Train V4 (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Teleport to Race Doors (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Complete Trial Race (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Kill Player After Trial (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Dojo Trainer (Extra)", Callback = function(s) _G.Dojoo = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Dragon Hunter (Extra)", Callback = function(s) _G.FarmBlazeEM = s end })
		Tabs.MainTab:Toggle({ Title = "Tween To Upgrade Droco Trial (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Drago (V1) (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Drago (V2) (Extra)", Callback = function(s) _G.AutoFireFlowers = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Drago (V3) (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Relic Drago Trial (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Train Drago v4 (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Tween to Drago Trials (Extra)", Callback = function(s) _G.TpDrago_Prehis = s end })
		Tabs.MainTab:Toggle({ Title = "Swap Drago Race (Extra)", Callback = function(s) _G.BuyDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Upgrade Dragon Talon With Uzoth (Extra)", Callback = function(s) _G.DT_Uzoth = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Craft Volcanic Magnet (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Find Prehistoric Island (Extra)", Callback = function(s) _G.Prehis_Find = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Patch Prehistoric Event (Extra)", Callback = function(s) _G.Prehis_Find = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Dino Bones (Extra)", Callback = function(s) _G.Prehis_Find = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Dragon Eggs (Extra)", Callback = function(s) _G.Prehis_Find = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Reset When Complete Volcano (Extra)", Callback = function(s) _G.UPGDrago = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Teleport Frozen Dimension (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Drive To Hydra Island (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Sail Boat (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Shark (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Piranha (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Terror Shark (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Fish Crew Member (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Haunted Crew Member (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Attack PirateGrandBrigade (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Attack Fish Boat (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Attack Sea Beast (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Attack Leviathan (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Find Kitsune Island (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Teleport to Shrine Actived (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Azure Ember (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Trade Azure Ember (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Select Dungeon Chip (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Start Raid (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Teleport To Lab (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Complete Raid [Safety] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Kill Aura (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Next Island (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Awakening (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Teleport to choose players (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Spectate Choose Players (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Aimbot Method Skills (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Aimbot Camera Closet Players (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Instance Mink V3 [ INF ] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Instance Energy [ INF ] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Instance Soru [ INF ] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Instance Observation Range [ INF ] (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Ignore Same Teams (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Accept Allies (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Berries (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Players (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Chests (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Fruits (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Island Location (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Flower (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Legendary Sword (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Aura Colour Dealers (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Gears (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp SeaEvent Island (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Esp Advanced Fruits Dealer (Extra)", Callback = function(s) _G.AutoEctoplasm = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Tween to NPCs (Extra)", Callback = function(s) _G.TPNpc = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Random Fruit (Extra)", Callback = function(s) _G.Random_Auto = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Drop Fruit (Extra)", Callback = function(s) _G.DropFruit = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Store Fruit (Extra)", Callback = function(s) _G.StoreF = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Tween to Fruit (Extra)", Callback = function(s) _G.TwFruits = s end })
		Tabs.MainTab:Toggle({ Title = "Auto Collect Fruit (Extra)", Callback = function(s) _G.InstanceF = s end })
		Tabs.MainTab:Toggle({ Title = "Unlock All Portals (Extra)", Callback = function(s) _G.PortalUnLock = s end })
		Tabs.MainTab:Toggle({ Title = "Turn on Time (Extra)", Callback = function(s) _G.daylightN = s end })
		Tabs.MainTab:Toggle({ Title = "Turn on Ice Walk (Extra)", Callback = function(s) _G.WalkWater = s end })
